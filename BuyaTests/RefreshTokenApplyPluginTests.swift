//
//  RefreshTokenApplyPluginTests.swift
//  BuyaTests
//
...
class RefreshTokenApplyPluginTests: XCTestCase {
    ...
    func testRefreshTokenApplyPlugin() {
        ...
        let plugin = RefreshTokenPlugin { () -> String? in
            return "test"
        }
        let provider = Buya.Provider<TestAPI>(requestBuilder: requestBuilder, plugins: [plugin], networkWorker: networkWorker)
        
        do {
            let data = try provider.request(TestAPI.fakeRequest).toBlocking().single()
            let currentQuery = String(data: data, encoding: .utf8)!
            
            XCTAssertEqual(currentQuery, "refreshToken=test")
        } catch {
...
extension RefreshTokenApplyPluginTests.TestAPI: Buya.EndpointType, Buya.RefreshTokenApply {
    var path: String {
        return "/api/test"
    }
